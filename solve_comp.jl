...
"""
    solve_comp(loc_I, loc_J, W, D, pc)

Solve the problem with complete adaptability.
"""
function solve_comp(loc_I, loc_J, W, D, pc)
    ...
    rm = Model(solver=GurobiSolver(OutputFlag=0))
    ...
    @variable(rm, 0 <= q[1:P,1:I,1:J] <= W, Int)
    # Slack variables to guarantee feasibility
    @variable(rm, 0 <= s[1:P, 1:J] <= D, Int)
    ...
    @constraint(rm, [p=1:P], z[p] >= 10*sum(s[p,j] for j in 1:J) + sum(c[i,j]*q[p,i,j] for i in 1:I, j in 1:J))
    ...
    getvalue(obj), round.(Int,getvalue(w)), round.(Int,U[worst]), round.(Int,getvalue(q[worst,:,:])), P
end

"""
    comp_adapt_solution(loc_I, loc_J, W, D, pc)

Run the solve function with complete adaptability and print results.
"""
function comp_adapt_solution(loc_I, loc_J, W, D, pc)

    obj, supp, dem, dist, plans = solve_comp(loc_I, loc_J, W, D, pc)
